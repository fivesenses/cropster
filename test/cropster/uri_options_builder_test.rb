...
class Cropster::UriOptionsBuilderTest < Test::Unit::TestCase
  ...
  test "should merge filter uri options" do
    uri = Cropster::UriOptionsBuilder.
      new("processings", { filter: { profile: "id" } }, "CROR").uri
  ...
  test "should build page options" do
    uri = Cropster::UriOptionsBuilder.
      new("processings", { filter: { profile: "id" }, page: { number: 1 } }, "CROR").uri
    result =
      "filter%5Bprocessings%5D%5Bgroup%5D=CROR&filter%5Bprocessings%5D%5Bprofile%5D=id&page%5Bnumber%5D=1&page%5Bsize%5D=50"
  ...
  test "should build options" do
    uri = Cropster::UriOptionsBuilder.
      new("processings", { filter: { profile: { id: 'FOO' } } }, "CROR").uri
  ...
  test "should build include options" do
    uri = Cropster::UriOptionsBuilder.
      new("lots", { include: "sourceContacts.all"}, "CROR").uri
